...
class Appointment(models.Model):
    ...
    end_datetime = fields.Datetime(
        string="End Datetime", compute="_compute_end_datetime", store=True)
    calendar_event_created = fields.Boolean(
        string='Calendar Event Created', default=False)
        ...
        for record in self:
            if record.start_datetime < fields.Datetime.now():
                raise ValidationError(
                    "The start datetime cannot be set in the past")
    ...
    def _check_unique_customer_appointment_date_time(self):
        for record in self:
            ...
            if conflict:
                raise ValidationError(
                    "The end datetime conflicts with another appointment.")
    ...
    def _check_quantity_selected_service(self):
        for record in self:
            if record.service_id and record.service_id.quantity <= 0:
                raise ValidationError(
                    "No available quantity for the selected service.")
